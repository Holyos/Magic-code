...
namespace Magic.Utils
{
    ...
    {
        public static Sprite[] loadAllSprite(string path, int resizeX =0, int resizeY =0, float offsetX = 0f, float offsetY = 0f,bool withFolders = false)
        {
        ...
        public static Sprite[] loadAllSprite(string path, float offsetX = 0f, float offsetY = 0f, bool withFolders = false)
        {
            string p = $"{Main.mainPath}/EmbededResources/{path}";
            DirectoryInfo folder = new DirectoryInfo(p);
            List<Sprite> res = new List<Sprite>();
